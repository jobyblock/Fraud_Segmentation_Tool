...
def evaluate_rules(df, string_df, rule_dict, threshold):
...
dtree_driver_new = run_query(f'select * from dtree_driver where {first_split_logic}')



performance_dict = evaluate_rules(dtree_driver_new, 'dtree_driver_new', final_dict, .05)

# COMMAND ----------

data = run_query('''select * from dtree_driver where (consumer_is_first_order not in ('false')) and (sp_c_order_attempt_cnt_d1 >= 3.5) and (whitepages_primary_address_checks_match_to_name in ('No match'))''')
...
# MAGIC
# MAGIC %autoreload 2
# MAGIC from functions import create_historical_decline_table, create_unique_decline_table
# MAGIC
